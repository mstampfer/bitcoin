...
BASH_COMPLETION_HEADER = """# Dynamic bash programmable completion for bitcoin-cli(1)
...
ZSH_COMPLETION_HEADER = """#compdef bitcoin-cli
...
class PossibleArgs():
    ...
    def generate_autocomplete(self, pos, shell='bash'):
...
class CliCompletionTest(BitcoinTestFramework):
    ...
    def skip_test_if_missing_module(self):
        ...
        # self.skip_if_no_bitcoind_zmq()  # Make ZMQ optional for completion generation
    ...
    def add_options(self, parser):
        parser.add_argument(
                ...
                default='bash',
                choices=['bash', 'zsh'],
                help='Shell type for completion generation (bash or zsh)',
        )

        parser.add_argument(
                '--header',
                help='Static header part of the completion file',
        )

        parser.add_argument(
                '--footer',
                help='Static footer part of the completion file',
        )

        parser.add_argument(
                '--completion',
                help='Location of the current completion file',
        )
    ...
    def generate_bash_completion_block(self, options):
        ...
        for cword in range(max_pos_options, 0, -1):
            ...
            for option in this_options:
                arg = option.generate_autocomplete(cword, 'bash')
    ...
    def generate_zsh_completion_block(self, options):
        ...
        for cword in range(max_pos_options, 0, -1):
            ...
            grouped_options = defaultdict(list)
            for option in this_options:
                arg = option.generate_autocomplete(cword, 'zsh')
                grouped_options[arg].append(option)
            ...
            if cword > 1:
                ...
                generated += f"    case \"${{words[CURRENT-1]}}\" in\n"
                indent = 8
    ...
    def generate_completion_block(self, options):
        """Wrapper to call the appropriate shell-specific generation."""
        if self.options.shell == 'zsh':
            return self.generate_zsh_completion_block(options)
        else:
            return self.generate_bash_completion_block(options)
    ...
    def generate_completion_file(self, commands):
        ...
        completion_header = ZSH_COMPLETION_HEADER if self.options.shell == 'zsh' else BASH_COMPLETION_HEADER
    ...
    def run_test(self):
        ...
        if self.options.shell == 'zsh':
            if self.options.header is None or len(self.options.header) == 0:
                self.options.header = path.join(test_data_dir, 'bitcoin-cli.header.zsh-completion')

            if self.options.footer is None or len(self.options.footer) == 0:
                self.options.footer = path.join(test_data_dir, 'bitcoin-cli.footer.zsh-completion')

            if self.options.completion is None or len(self.options.completion) == 0:
                self.options.completion = path.join(src_dir, 'contrib', 'completions', 'zsh', '_bitcoin-cli')
        else:
            if self.options.header is None or len(self.options.header) == 0:
        ...
        self.log.info(f'Generating new {self.options.shell} completion file...')
        commands = self.generate_completion_block(commands)
        new_completion = self.generate_completion_file(commands)

        if self.options.overwrite:
            self.log.info(f"Overwriting the {self.options.shell} completion file...")
            self.write_completion_file(new_completion)

        self.log.info(f'Checking if the generated and the original {self.options.shell} completion files match...')
